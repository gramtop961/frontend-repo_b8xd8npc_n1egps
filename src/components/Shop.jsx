  ...
  return (
    <div className="max-w-6xl mx-auto p-4 grid md:grid-cols-3 gap-6">
      <div className="md:col-span-2 space-y-8">
        <section>
          <h2 className="text-xl font-semibold mb-3">Măcelărie</h2>
            ...
            {butcher.map((b) => (
              <div key={b._id} className="border rounded-lg p-3 bg-white">
                ...
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <button onClick={() => addButcher(b, document.getElementById(`w-${b._id}`).value)} className="px-3 py-1.5 bg-gray-900 text-white rounded">Adaugă</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <section>
          <h2 className="text-xl font-semibold mb-3">Bacanie Turcească</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {grocery.map((g) => (
              <div key={g._id} className="border rounded-lg p-3 bg-white">
                ...
                <div className="mt-2 text-emerald-600 font-semibold">{g.price} / bucată</div>
                <div className="mt-3 flex items-center gap-2">
                  <input type="number" min="1" step="1" defaultValue={1} className="border rounded px-2 py-1 w-20" id={`q-${g._id}`}/>
                  <button onClick={() => addGrocery(g, document.getElementById(`q-${g._id}`).value)} className="px-3 py-1.5 bg-gray-900 text-white rounded">Adaugă</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <aside className="bg-white border rounded-lg p-4 h-fit sticky top-24">
        <h3 className="font-semibold mb-3">Coșul tău</h3>
        <div className="space-y-2 max-h-80 overflow-auto pr-1">
          {cart.length === 0 && <div className="text-sm text-gray-500">Fără produse</div>}
          {cart.map((c, i) => (
...
function Checkout({ cart, setCart, total }) {
  ...
  const [pay, setPay] = useState("Plata numerar la livrare");
  ...
  async function placeOrder() {
    ...
    const body = {
      ...
      phone,
      address,
      payment_method: pay === "Card la livrare" ? "Card on Delivery" : "Cash on Delivery",
      items: cart,
      total,
    };
    try {
      ...
      if (res.ok) {
        setStatus("Comandă plasată! ID " + data._id);
        setCart([]);
        setName(""); setPhone(""); setAddress("");
      } else {
        setStatus(data.detail || "Eșec la plasarea comenzii");
      }
    } catch (e) {
  ...
  return (
    <div className="mt-4 space-y-2">
      <div className="text-sm text-gray-600">Finalizare comandă</div>
      <input className="w-full border rounded px-2 py-1" placeholder="Nume" value={name} onChange={(e)=>setName(e.target.value)} />
      <input className="w-full border rounded px-2 py-1" placeholder="Telefon" value={phone} onChange={(e)=>setPhone(e.target.value)} />
      <textarea className="w-full border rounded px-2 py-1" placeholder="Adresă" value={address} onChange={(e)=>setAddress(e.target.value)} />
      <select className="w-full border rounded px-2 py-1" value={pay} onChange={(e)=>setPay(e.target.value)}>
        <option>Plata numerar la livrare</option>
        <option>Card la livrare</option>
      </select>
      <button onClick={placeOrder} className="w-full bg-rose-600 text-white rounded py-2">Plasează comanda</button>
